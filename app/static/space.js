...
function renderProfile(data) {
  ...
  if (data.is_owner) {
    ...
    follow.type = "button";
    follow.className = "community-btn primary";
    follow.dataset.followUser = user.user_id || user.id || profileIDFromPath();
    follow.dataset.following = data.following ? "1" : "0";
    follow.textContent = data.following ? "已关注" : "关注";
    actions.appendChild(follow);
    const msg = createNode("a", "community-btn", "发私信");
    msg.href = "/messages";
...
function bindFollowButtons() {
  document.querySelectorAll("[data-follow-user]").forEach((btn) => {
    ...
    btn.addEventListener("click", async () => {
      try {
        const data = await fetchJSON(`/api/social/follows/${encodeURIComponent(userId)}`, { method: following ? "DELETE" : "POST" });
        following = Boolean(data.following);
        btn.dataset.following = following ? "1" : "0";
        btn.textContent = following ? "已关注" : "关注";
      } catch (err) {
        btn.title = err.message || "操作失败";
      }
    });
  });
}
